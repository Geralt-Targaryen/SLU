...
from utils.example_t5 import Example_t5
from utils.batch_t5 import from_example_list
from model.mt5_seq2seq import SLUTagging_t5
...
Example_t5.configuration(args.dataroot, model='google/mt5-base')
train_dataset = Example_t5.load_dataset(train_path)               # list[Example]
dev_dataset = Example_t5.load_dataset(dev_path)
print("Load dataset and database finished, cost %.4fs ..." % (time.time() - start_time))
print("Dataset size: train -> %d ; dev -> %d" % (len(train_dataset), len(dev_dataset)))

args.vocab_size = Example_t5.tokenizer.vocab_size                   # baseline: 1741, chinese-roberta: 21128
args.tokenizer = Example_t5.tokenizer

model: SLUTagging_t5 = SLUTagging_t5(args, model='google/mt5-base').to(device)
...
def decode(choice):
    ...
    with torch.no_grad():
        for i in range(0, len(dataset), args.batch_size):
            ...
            total_loss += loss
            count += 1
        metrics = Example_t5.evaluator.acc(predictions, labels)
    gc.collect()
...
def predict():
    ...
    test_dataset = Example_t5.load_dataset(test_path)
    predictions = []
    with torch.no_grad():
...
if not args.testing:
    ...
    for i in range(args.max_epoch):
        ...
        if dev_acc > best_result['dev_acc']:
            ...
            torch.save(model.state_dict(), open(args.checkpoint, 'wb'))
